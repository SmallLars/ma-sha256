...
//    solver.log_to_file("solver.txt");
    ...
    for (unsigned r = 513; r <= assumptions.size(); r++) {
        cout << setw(3) << r - 512 << " / " << assumptions.size() - 512 << ":" << flush;
        ...
        cout << "\n  Initialwert:";
        for (unsigned i = 0; i < 8; i++) {
            printf(" %08lx", solver_readInt(solver, (i + 16) * 32, 32));
        }
        cout << "\n";

        char red_name[24];
        char irred_name[22];
        sprintf(red_name, "dump/%03u_learned.dimacs", r - 512);
        sprintf(irred_name, "dump/%03u_irred.dimacs", r - 512);
