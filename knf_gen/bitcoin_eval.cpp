...
int main(int argc, const char* argv[]) {
    int leadingZeros;
    if (argc < 2 || (leadingZeros = atoi(argv[1])) < 1) {
        cout << "sha256_eval <leadingZeros>\nleadingZeros needs to be > 0\n";
        return 0;
    }
...
//    sha256_calc(state_2, input_2);
    ...
    long numCPU = 1; // sysconf( _SC_NPROCESSORS_ONLN );
    cout << "CPUNUM: " << numCPU << "\n";
...
//    solver.log_to_file("solver.txt");
    ...
    time(&rawtime);
    char filename[64];
    strftime(filename, 64, "bitcoin_eval %Y-%m-%d %H:%M:%S", localtime(&rawtime));
    ...
    for (int r = leadingZeros; r <= leadingZeros; r++) {
